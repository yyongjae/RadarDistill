...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = [
            'vfe', 'radar_vfe', 'backbone_3d', 'radar_backbone_3d', 'map_to_bev_module', 'radar_map_to_bev_module', 'pfe',
            'backbone_2d', 'radar_backbone_2d', 'dense_head','radar_dense_head', 'point_head', 'roi_head'
        ]
    ...
    def build_networks(self):
        ...
        if hasattr(self.dataset.point_feature_encoder,'radar_num_point_features') :
            model_info_dict.update({
                'radar_num_rawpoint_features' : self.dataset.point_feature_encoder.radar_num_point_features,
                'radar_num_point_features' : self.dataset.point_feature_encoder.radar_num_point_features
            }
            )
    ...
    def build_radar_vfe(self, model_info_dict):
        ...
        model_info_dict['radar_num_point_features'] = radar_vfe_module.get_output_feature_dim()
    ...
    def build_radar_backbone_3d(self, model_info_dict):
        ...
        radar_backbone_3d_module = backbones_3d.__all__[self.model_cfg.RADAR_BACKBONE_3D.NAME](
            ...
            input_channels=model_info_dict['radar_num_point_features'],
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range']
        )
        model_info_dict['module_list'].append(radar_backbone_3d_module)
        model_info_dict['radar_num_point_features'] = radar_backbone_3d_module.num_point_features
        model_info_dict['radar_backbone_channels'] = radar_backbone_3d_module.backbone_channels \
            if hasattr(radar_backbone_3d_module, 'backbone_channels') else None
    ...
    def build_radar_map_to_bev_module(self, model_info_dict):
        ...
        model_info_dict['radar_num_bev_features'] = radar_map_to_bev_module.num_bev_features
    ...
    def build_radar_dense_head(self, model_info_dict):
        if self.model_cfg.get('RADAR_DENSE_HEAD', None) is None:
            return None, model_info_dict
        radar_dense_head_module = dense_heads.__all__[self.model_cfg.RADAR_DENSE_HEAD.NAME](
            model_cfg=self.model_cfg.RADAR_DENSE_HEAD,
            input_channels=model_info_dict['radar_num_bev_features'] if 'radar_num_bev_features' in model_info_dict else self.model_cfg.RADAR_DENSE_HEAD.INPUT_FEATURES,
            num_class=self.num_class if not self.model_cfg.RADAR_DENSE_HEAD.CLASS_AGNOSTIC else 1,
            class_names=self.class_names,
            grid_size=model_info_dict['grid_size'],
            point_cloud_range=model_info_dict['point_cloud_range'],
            predict_boxes_when_training=self.model_cfg.get('ROI_HEAD', False),
        ...
        model_info_dict['module_list'].append(radar_dense_head_module)
        return radar_dense_head_module, model_info_dict
