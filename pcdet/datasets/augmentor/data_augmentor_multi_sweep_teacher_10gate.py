...
from . import augmentor_utils, database_sampler, database_sampler_distill, database_sampler_radar, database_sampler_distill_multi_sweep_teacher


class DataAugmentor_Multi_Sweep_Teacher(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None):
        ...
        self.TEACHER_KEYS = ('teacher_points_s3','teacher_points_s4','teacher_points_s5','teacher_points_s6','teacher_points_s7','teacher_points_s8', 'teacher_points_s9', 'teacher_points_s10')
    ...
    def gt_sampling_distill(self, config=None):
        db_sampler = database_sampler_distill_multi_sweep_teacher.DataBaseSampler_Distill_Multi_Sweep_Teacher(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
    ...
    def gt_sampling_radar(self, config=None):
        db_sampler = database_sampler_radar.DataBaseSampler_Radar(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
    ...
    def _teacher_apply_flip(self, data_dict, axis, enable: bool):
        ...
        for k in self.TEACHER_KEYS:
            if k not in data_dict or data_dict[k] is None or data_dict[k].size == 0:
                continue
            pts = data_dict[k]
            if axis == 'x':   # OpenPCDet conv: flip-x => y -> -y
                pts[:, 1] = -pts[:, 1]
            elif axis == 'y': # flip-y => x -> -x
                pts[:, 0] = -pts[:, 0]
            data_dict[k] = pts.astype(np.float32)

    def _teacher_apply_rotation(self, data_dict, angle: float):
        if angle is None: 
            return
    ...
    def _teacher_apply_translation(self, data_dict, t: np.ndarray):
        if t is None: 
            return
    ...
    def random_world_flip_distill(self, data_dict=None, config=None): # 이거 사용
        if data_dict is None:
            return partial(self.random_world_flip_distill, config=config)

        gt_boxes = data_dict['gt_boxes']
        radar_points = data_dict['radar_points']
        dummy_pts = np.zeros((0, 4), dtype=np.float32)

        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            # enable만 얻기 위해 dummy_pts 사용
            gt_boxes, _, radar_points, enable = getattr(augmentor_utils, f'random_flip_distill_along_{cur_axis}')(
                gt_boxes, dummy_pts, radar_points, return_flip=True
            )
            data_dict[f'flip_{cur_axis}'] = enable

            if 'roi_boxes' in data_dict:
                num_frame, num_rois, dim = data_dict['roi_boxes'].shape
                roi_boxes, _, _ = getattr(augmentor_utils, f'random_flip_along_{cur_axis}')(
                    data_dict['roi_boxes'].reshape(-1, dim), np.zeros([1, 3]),
                    return_flip=True, enable=enable
                )
                data_dict['roi_boxes'] = roi_boxes.reshape(num_frame, num_rois, dim)

            # [NEW]
            self._teacher_apply_flip(data_dict, cur_axis, enable)
    ...
    def random_world_rotation_distill(self, data_dict=None, config=None): # 이거 사용
        if data_dict is None:
            return partial(self.random_world_rotation_distill, config=config)
        ...
        dummy_pts = np.zeros((0, 4), dtype=np.float32)
        gt_boxes, _, radar_points, noise_rot = augmentor_utils.global_rotation_distill(
            data_dict['gt_boxes'], dummy_pts, data_dict['radar_points'],
        ...
        if 'roi_boxes' in data_dict:
            ...
            roi_boxes, _, _ = augmentor_utils.global_rotation(
                data_dict['roi_boxes'].reshape(-1, dim), np.zeros([1, 3]),
                rot_range=rot_range, return_rot=True, noise_rotation=noise_rot
            )
            data_dict['roi_boxes'] = roi_boxes.reshape(num_frame, num_rois, dim)

        # [NEW]
        self._teacher_apply_rotation(data_dict, noise_rot)
    ...
    def random_world_scaling_distill(self, data_dict=None, config=None): # 이거 사용
        if data_dict is None:
            return partial(self.random_world_scaling_distill, config=config)

        dummy_pts = np.zeros((0, 4), dtype=np.float32)
        if 'roi_boxes' in data_dict:
            gt_boxes, roi_boxes, _, noise_scale = augmentor_utils.global_scaling_with_roi_boxes(
                data_dict['gt_boxes'], data_dict['roi_boxes'], dummy_pts,
                config['WORLD_SCALE_RANGE'], return_scale=True
            )
            data_dict['roi_boxes'] = roi_boxes
            radar_points = data_dict['radar_points']
        else:
            gt_boxes, _, radar_points, noise_scale = augmentor_utils.global_scaling_distill(
                data_dict['gt_boxes'], dummy_pts, data_dict['radar_points'],
                config['WORLD_SCALE_RANGE'], return_scale=True
            )

        # [NEW]
        self._teacher_apply_scaling(data_dict, noise_scale)
    ...
    def random_world_translation_distill(self, data_dict=None, config=None): # 이거 사용
        if data_dict is None:
            return partial(self.random_world_translation_distill, config=config)
        ...
        # 적용
        gt_boxes[:, :3] += noise_translate
        radar_points[:, :3] += noise_translate
        if 'roi_boxes' in data_dict:
            data_dict['roi_boxes'][:, :3] += noise_translate

        # [NEW] teacher 3종 동일 translation
        self._teacher_apply_translation(data_dict, noise_translate.squeeze(0))
