...
from . import augmentor_utils, database_sampler, database_sampler_distill, database_sampler_radar
...
class DataAugmentor(object):
    ...
    def gt_sampling_distill(self, config=None):
        db_sampler = database_sampler_distill.DataBaseSampler_Distill(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
    ...
    def random_world_flip_distill(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_world_flip_distill, config=config)
        gt_boxes, points, radar_points = data_dict['gt_boxes'], data_dict['points'], data_dict['radar_points']
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['x', 'y']
            gt_boxes, points,radar_points, enable = getattr(augmentor_utils, 'random_flip_distill_along_%s' % cur_axis)(
                gt_boxes, points,radar_points, return_flip=True
            )
            data_dict['flip_%s'%cur_axis] = enable
            if 'roi_boxes' in data_dict.keys():
                num_frame, num_rois,dim = data_dict['roi_boxes'].shape
    ...
    def random_world_rotation_distill(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_world_rotation_distill, config=config)
        rot_range = config['WORLD_ROT_ANGLE']
        if not isinstance(rot_range, list):
            rot_range = [-rot_range, rot_range]
        gt_boxes, points,radar_points, noise_rot = augmentor_utils.global_rotation_distill(
            data_dict['gt_boxes'], data_dict['points'],data_dict['radar_points'], rot_range=rot_range, return_rot=True
        )
        if 'roi_boxes' in data_dict.keys():
            num_frame, num_rois,dim = data_dict['roi_boxes'].shape
            roi_boxes, _, _ = augmentor_utils.global_rotation(
    ...
    def random_world_scaling_distill(self, data_dict=None, config=None):
        if data_dict is None:
        ...
        if 'roi_boxes' in data_dict.keys():
            ...
            gt_boxes, points, radar_points, noise_scale = augmentor_utils.global_scaling_distill(
                data_dict['gt_boxes'], data_dict['points'], data_dict['radar_points'], config['WORLD_SCALE_RANGE'], return_scale=True
            )
    ...
    def random_world_translation_distill(self, data_dict=None, config=None):
        if data_dict is None:
