...
class Radar_DynamicPillarVFESimple2D(DynamicPillarVFESimple2D):
    def forward(self, batch_dict, **kwargs):
        ...
        batch_dict['radar_pillar_coords'] = pillar_coords
        return batch_dict


class DynamicPillarVFESimple2D_KY(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
    ...
    def forward(self, batch_dict, **kwargs):
        if self.double_flip:
            batch_dict['frame_id'] = np.array([batch_dict['frame_id'][0]]*4)
            batch_dict['batch_size'] = batch_dict['batch_size']*4
            batch_dict['metadata'] = np.array([batch_dict['metadata'][0]]*4)
        points = batch_dict['points']  # (batch_idx, x, y, z, i, e)
        points_coords = torch.floor(
            (points[:, [1, 2]] - self.point_cloud_range[[0, 1]]) / self.voxel_size[[0, 1]]).int()
        mask = ((points_coords >= 0) & (points_coords < self.grid_size[[0, 1]])).all(dim=1)
...
class Radar_DynamicPillarVFESimple2D_KY(DynamicPillarVFESimple2D_KY):
    def forward(self, batch_dict, **kwargs):
        if self.double_flip:
            batch_dict['frame_id'] = np.array([batch_dict['frame_id'][0]]*4)
            batch_dict['batch_size'] = batch_dict['batch_size']*4
