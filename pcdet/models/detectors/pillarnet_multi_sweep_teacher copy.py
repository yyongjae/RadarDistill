...
class PillarNet_Multi_Sweep_Teacher(Detector3DTemplate):
    ...
    def _forward_teacher_once(self, batch_dict, points_key: str):
        ...
        # Debug teacher forward pass
        for i, idx in enumerate(self.teacher_feat_mod_idx):  # teacher VFE/백본/넥/헤드
            module_name = self.module_list[idx].__class__.__name__
            tbd = self.module_list[idx](tbd)
            
            # Check for NaN after each teacher module
            if 'spatial_features_2d' in tbd:
                feat = tbd['spatial_features_2d']
                if hasattr(feat, 'features'):  # SparseConvTensor
                    if torch.isnan(feat.features).any():
                        print(f"[TEACHER DEBUG] NaN in spatial_features_2d.features after {module_name} (idx={idx})")
                        break
                elif isinstance(feat, torch.Tensor):
                    if torch.isnan(feat).any():
                        print(f"[TEACHER DEBUG] NaN in spatial_features_2d after {module_name} (idx={idx})")
                        break
                        
            if 'multi_scale_2d_features' in tbd:
                for key, feat in tbd['multi_scale_2d_features'].items():
                    if hasattr(feat, 'features'):  # SparseConvTensor
                        if torch.isnan(feat.features).any():
                            print(f"[TEACHER DEBUG] NaN in {key}.features after {module_name} (idx={idx})")
                            break
                    elif isinstance(feat, torch.Tensor):
                        if torch.isnan(feat).any():
                            print(f"[TEACHER DEBUG] NaN in {key} after {module_name} (idx={idx})")
                            break
    ...
    def _build_teacher_stacks_inplace(self, batch_dict):
        ...
        keys = ['teacher_points_s3','teacher_points_s4','teacher_points_s5','teacher_points_s6','teacher_points_s7','teacher_points_s8', 'teacher_points_s9', 'teacher_points_s10']
        lows, highs, highs8 = [], [], []
        for k in keys:
    ...
    def forward(self, batch_dict):
        ...
        for idx, cur_module in enumerate(self.module_list):
            ...
            # Debug student modules for NaN
            if self.training and self.model_cfg.get('DISTILL', False):
                if 'radar_spatial_features_2d' in batch_dict:
                    feat = batch_dict['radar_spatial_features_2d']
                    if hasattr(feat, 'features'):  # SparseConvTensor
                        if torch.isnan(feat.features).any():
                            print(f"[STUDENT MODULE DEBUG] NaN in radar_spatial_features_2d.features after {cur_name} (idx={idx})")
                            break
                    elif isinstance(feat, torch.Tensor):
                        if torch.isnan(feat).any():
                            print(f"[STUDENT MODULE DEBUG] NaN in radar_spatial_features_2d after {cur_name} (idx={idx})")
                            break
                            
                if 'radar_multi_scale_2d_features' in batch_dict:
                    for key, feat in batch_dict['radar_multi_scale_2d_features'].items():
                        if hasattr(feat, 'features'):  # SparseConvTensor
                            if torch.isnan(feat.features).any():
                                print(f"[STUDENT MODULE DEBUG] NaN in {key}.features after {cur_name} (idx={idx})")
                                break
                        elif isinstance(feat, torch.Tensor):
                            if torch.isnan(feat).any():
                                print(f"[STUDENT MODULE DEBUG] NaN in {key} after {cur_name} (idx={idx})")
                                break
