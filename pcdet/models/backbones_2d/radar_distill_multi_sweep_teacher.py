...
class Radar_Distill_Multi_Sweep_Teacher(BaseBEVBackboneV2):
    def __init__(self, model_cfg, **kwargs):
        ...
        # [CHANGED] Ver3 게이터 생성 (원본 temp=0.7 유지, 스윕=3)
        # 초기 모드는 원본과 최대한 유사하게 'soft'로 시작하고, 학습 중 auto_step으로 하드화
        self.gater_low  = SweepGaterV3(
            C=256, num_sweeps=3, temp=0.7, use_adapter=True,
            mode='soft', k=2, lb_coeff=0.0, ent_coeff=0.0, tv_coeff=0.0,  # 보조손실은 로깅만 (loss에 미반영)
            learn_router=True
        )
    ...
    def get_loss(self, batch_dict):
        ...
        # LiDAR teachers (3-sweep stacks)
        T_low   = batch_dict['lidar_teachers_low']   # [B,3,C,H8,W8]
        T_high  = batch_dict['lidar_teachers_high']  # [B,3,C,H, W]
