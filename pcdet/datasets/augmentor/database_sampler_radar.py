...
class DataBaseSampler_Radar(object):
    ...
    def filter_by_min_radar_points(self, db_infos, min_gt_points_list):
        for name_num in min_gt_points_list:
            ...
            if min_num > 0 and name in db_infos.keys():
                ...
                for info in db_infos[name]:
                    if info['num_radar_points_in_gt'] >= min_num:
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                file_path = self.root_path / info['radar_path']

                obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(
                    [-1, self.sampler_cfg.NUM_POINT_FEATURES])
                if obj_points.shape[0] != info['num_radar_points_in_gt']:
                    obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(-1, self.sampler_cfg.NUM_POINT_FEATURES)

            assert obj_points.shape[0] == info['num_radar_points_in_gt']
