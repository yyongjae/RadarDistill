...
class DataProcessor(object):
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        if data_dict.get('radar_points', None) is not None:
            ...
            radar_points = data_dict['radar_points']
            radar_voxel_output = self.radar_voxel_generator.generate(radar_points)
            ...
            if config.get('DOUBLE_FLIP', False):
                radar_voxels_list, radar_voxel_coords_list, radar_voxel_num_points_list = [radar_voxels], [radar_coordinates], [radar_num_points]
                radar_points_yflip, radar_points_xflip, radar_points_xyflip = self.double_flip(radar_points)
                radar_points_list = [radar_points_yflip, radar_points_xflip, radar_points_xyflip]
                keys = ['yflip', 'xflip', 'xyflip']
                for i, key in enumerate(keys):
                    radar_voxel_output = self.voxel_generator.generate(radar_points_list[i])
                    radar_voxels, radar_coordinates, radar_num_points = radar_voxel_output

                    if not data_dict['use_lead_xyz']:
                        radar_voxels = radar_voxels[..., 3:]
                    radar_voxels_list.append(radar_voxels)
                    radar_voxel_coords_list.append(radar_coordinates)
                    radar_voxel_num_points_list.append(radar_num_points)

                data_dict['radar_voxels'] = radar_voxels_list
                data_dict['radar_voxel_coords'] = radar_voxel_coords_list
                data_dict['radar_voxel_num_points'] = radar_voxel_num_points_list
            else:
